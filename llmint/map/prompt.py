...
5. If the target schema contains an optional field, you can either do a normal translation from a source field or use the addOptionalFunction.
6. All optional fields will have default values, so you will always need to call the setDefaultFunction when an optional field exists in the target schema.
...
8. If you change the type of a source field, also consider using the complexConversionFunction to specify the equation needed before casting. But only use conversions specified by the user or mathematically proved. Do not assume or use any "typical" or "common" thresholds.
9. If the input schemas are for different kinds of products, use the incompatibleFunction and return immediately. Do not generate any other mappings."""
